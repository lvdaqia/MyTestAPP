...
public class JTT905Handler extends SimpleChannelInboundHandler<JTT905Bean> {
    private static final String TAG = "JTT905Handler";
    private JTT905Client jtt905Client;
    private OnConnectionListener listener;

    public JTT905Handler(JTT905Client jtt905Client, OnConnectionListener listener) {
        this.jtt905Client = jtt905Client;
        this.listener = listener;
    }

    @Override
    protected void channelRead0(ChannelHandlerContext ctx, JTT905Bean bean) throws Exception {
        /*if (listener != null) {
    ...
    private void terminalControl(JTT905Bean bean){
        JTT905Bean.MsgHeader msgHeader = bean.getMsgHeader();
        JTT905Bean authBean = JTT808Util.universalResponse(msgHeader.getFlowNum(), msgHeader.getMsgId(),false);
        JTT905Client.getInstance().writeAndFlush(authBean);
    ...
    private void textdelivery(JTT905Bean bean){
        ...
        listener.textdelivery(JTT905Client.getInstance(),text);
    }

    private void JT905_0x0104(JTT905Bean bean) {
        JTT808Util.JT905_0x0104(bean);
    }

    private void queryAtrribute(){
        listener.queryAtrribute(JTT905Client.getInstance());
    }

    private void update(JTT905Bean bean){
        ByteBuf msgBody = bean.getMsgBody();
    ...
    private void transferMediaInfo(JTT905Bean bean) {
        JTT808Bean mediaInfo = JTT808Util.getMediaInfo();
        JTT808Client.getInstance().writeAndFlush(mediaInfo);
    }
     ...
     * @param bean
     */
    private void registerResult(JTT905Bean bean) {
        ByteBuf msgBody = bean.getMsgBody();
        ...
        Log.e(TAG,"鉴权码：" + HexUtil.byte2HexStr(authCode));
        JTT905Bean authBean = JTT808Util.auth(authCode,false);
     ...
     * @param bean
     */
    private void universalResult(JTT905Bean bean) {
        ByteBuf msgBody = bean.getMsgBody();
        short flowNum = msgBody.readShort();
     ...
     * @param bean
     */
    private void uploadFiles(JTT905Bean bean) {
        //响应平台
        JTT905Bean.MsgHeader msgHeader = bean.getMsgHeader();
        JTT905Bean authBean = JTT808Util.universalResponse(msgHeader.getFlowNum(), msgHeader.getMsgId(),false);
        JTT905Client.getInstance().writeAndFlush(authBean);
        ...
        listener.UpVedioFile(user,passwor,ip,port,channelNum,vediotype,starttime,stoptime, filePath);
//        //udp端口用不到，先忽略
//        body.readBytes(2);
//        //报警标识号
...
//        jtt1078Client.connect();
     ...
     * @param bean
     */
    private void terminalParams(JTT905Bean bean) throws Exception {
     ...
     * @param bean
     */
    private void audioVideoLive(JTT905Bean bean) {
        ...
        JTT905Bean jtt905Bean = JTT808Util.universalResponse(msgHeader.getFlowNum(), msgHeader.getMsgId(),false);
        JTT905Client.getInstance().writeAndFlush(jtt905Bean);
        Log.d(TAG, "响应了服务器的实时音视频请求：ip = " + ip + ":" + tcpPort +" dataType = " + dataType+" channelNum:"+channelNum);

        if (listener != null) {
            listener.audioVideoLive905(ip, tcpPort, channelNum, dataType);
        }
    }
     ...
     * @param bean
     */
    private void audioVideoLiveControl(JTT905Bean bean) {
        ByteBuf body = bean.getMsgBody();
        byte channelNum = body.readByte();
        ...
        if (listener != null) {
            listener.audioVideo905LiveControl(channelNum, control, closeAudio, switchStream);
        }
    }
     ...
     * jtt1078，音视频实传输状态通知
     */
    private void audioVideoLiveState(JTT905Bean bean) {
        ByteBuf body = bean.getMsgBody();
        byte channelNum = body.readByte();
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (listener != null) {
            listener.on905ConnectionSateChange(OnConnectionListener.CONNECTED);
        }
    }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        ...
        if (evt instanceof IdleStateEvent) {
            if(((IdleStateEvent)evt).state() == IdleState.READER_IDLE){
                Log.d(TAG,"长期没收收到服务器的回复");
                //reconnect
                //jtt808Client.reConnect();
            }
            if (((IdleStateEvent) evt).state() == IdleState.WRITER_IDLE) {
                JTT905Bean heartBeatBean = JTT808Util.heartBeat(false);
     ...
     * @param bean
     */
    private void localVideoList(JTT905Bean bean) {
        ByteBuf body = bean.getMsgBody();
        ...
        listener.localVideoList(bean.getMsgHeader().getFlowNum(), channelNum, starttime, stoptime, alarmSign, dataType, bitstreamType, storageType);
    }
     ...
     * @param bean
     */
    private void upLocalVideo(JTT905Bean bean) {
        ByteBuf body = bean.getMsgBody();
        JTT905Bean.MsgHeader msgHeader = bean.getMsgHeader();
        ...
        if (listener != null) {
            ...
            listener.PlayBackAudioVideoLive(ip, tcpPort, channelNum, dataType,starttime,stoptime);
        }
    }
    /**
     * jtt1078，音视频实时传输控制
     ...
     * @param bean
     */
    private void PlayBackVideoAudioLiveControl(JTT905Bean bean) {
        ByteBuf body = bean.getMsgBody();
        byte channelNum = body.readByte();
        byte control = body.readByte();
